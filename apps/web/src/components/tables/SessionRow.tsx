    ...
    >
      ...
      <td className="px-4 py-4 align-middle">
        <div className="min-w-0">
          <div className="flex items-center gap-2 truncate font-medium text-slate-100">
            <span className="text-slate-500">{expanded ? "▾" : "▸"}</span>
            {traceLabel(trace.externalId, trace.id)}
          </div>
          <div className="mt-1 text-xs text-slate-500">{formatDateTime(trace.startedAt)}</div>
        </div>
      </td>
      <td className="truncate px-4 py-4 text-sm text-slate-300 align-middle">
        {trace.model ?? "unknown"}
      </td>
      <td className="px-4 py-4 text-right text-sm text-slate-400 align-middle">
        {formatCompactInt(trace.spanCount)}
      </td>
      <td className="px-4 py-4 text-right text-sm text-slate-300 align-middle">
        {formatCompactInt(trace.totalInputTokens)}
      </td>
      <td className="px-4 py-4 text-right text-sm text-slate-300 align-middle">
        {formatCompactInt(trace.totalOutputTokens)}
      </td>
      <td className="px-4 py-4 text-right text-sm font-medium text-slate-100 align-middle">
        {formatUsd(trace.totalCostUsd)}
      </td>
      <td
        className={`px-4 py-4 text-right text-sm font-medium align-middle ${
          trace.wasteUsd > 0 ? "text-red-300" : "text-slate-500"
        }`}
      >
        {formatUsd(trace.wasteUsd)}
      </td>
    </tr>
  );
}
